...
function ConversationItem({
  ...
  const itemSurfaceClass = isActive
    ? "bg-clay-peach/45 text-clay-ink"
    : "bg-transparent text-clay-body hover:bg-clay-surface-soft/80";
      ...
      >
        ...
        {!isEditing && (
          ...
          >
            ...
            {menuOpen && (
              <div
                className="absolute top-full right-0 z-50 mt-1 min-w-[9.5rem] overflow-hidden rounded-[var(--radius-md)] border border-clay-hairline/80 bg-clay-canvas py-1 shadow-lg"
                role="menu"
              >
                <button
                  type="button"
                  ...
                  className="flex w-full px-3 py-2 text-left text-sm text-clay-body hover:bg-clay-surface-soft/90"
                >
                  {isPinned ? "고정 해제" : "고정"}
                </button>
                <button
...
function ConversationSection({
  ...
  return (
    <div className="min-h-0">
      {title && (
        <p className="px-3 pb-1.5 text-xs text-clay-muted-soft">{title}</p>
      )}
...
export function Sidebar({
  ...
  return (
    <aside
      className={`flex shrink-0 flex-col overflow-hidden border-r border-clay-lavender/35 bg-clay-surface-strong/95 backdrop-blur-md transition-[width] duration-300 ease-in-out ${
        open ? "w-72" : "w-0 border-r-0"
      }`}
      aria-hidden={!open}
    >
      <div className="flex h-full w-72 flex-col">
        <div className="flex items-center justify-between gap-3 border-b border-clay-lavender/20 px-4 py-4">
